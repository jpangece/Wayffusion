...
RUN_TIMESTAMP="${RUN_TIMESTAMP:-$(date +%Y%m%d_%H%M%S)_mappo_pure_all4}"
...
GOAL_CONFIG="${GOAL_CONFIG:-configs/policy/mappo_shared_long.yaml}"
COVERAGE_CONFIG="${COVERAGE_CONFIG:-configs/policy/mappo_shared_long.yaml}"
RISK_CONFIG="${RISK_CONFIG:-configs/policy/mappo_shared_long.yaml}"
FORMATION_CONFIG="${FORMATION_CONFIG:-configs/policy/mappo_shared_long.yaml}"
...
write_summary_header() {
  printf 'label,status,exit_code,start_time,end_time,duration_sec,task,total_updates,eval_episodes,config,env_config,run_dir,log_path\n' > "${SUMMARY_CSV}"
}

append_summary() {
  local label="$1" status="$2" exit_code="$3" start_time="$4" end_time="$5" duration_sec="$6" task="$7" total_updates="$8" eval_episodes="$9" config="${10}" env_config="${11}" run_dir="${12}" log_path="${13}"
  printf '"%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s","%s"\n' \
    "${label}" "${status}" "${exit_code}" "${start_time}" "${end_time}" "${duration_sec}" "${task}" "${total_updates}" "${eval_episodes}" "${config}" "${env_config}" "${run_dir}" "${log_path}" >> "${SUMMARY_CSV}"
}
...
# enabled|label|task|total_updates|config|env_config|env_backend|cuda_visible_devices|extra_args
JOBS=(
  "1|goal_nav_mappo_pure|goal_nav|${GOAL_TOTAL_UPDATES}|${GOAL_CONFIG}|${GOAL_ENV_CONFIG}|${GOAL_ENV_BACKEND:-${DEFAULT_ENV_BACKEND}}|${GOAL_CUDA_VISIBLE_DEVICES:-${DEFAULT_CUDA_VISIBLE_DEVICES}}|${GOAL_EXTRA_ARGS:-}"
  "1|coverage_mappo_pure|coverage|${COVERAGE_TOTAL_UPDATES}|${COVERAGE_CONFIG}|${COVERAGE_ENV_CONFIG}|${COVERAGE_ENV_BACKEND:-${DEFAULT_ENV_BACKEND}}|${COVERAGE_CUDA_VISIBLE_DEVICES:-${DEFAULT_CUDA_VISIBLE_DEVICES}}|${COVERAGE_EXTRA_ARGS:-}"
  "1|risk_nav_mappo_pure|risk_nav|${RISK_TOTAL_UPDATES}|${RISK_CONFIG}|${RISK_ENV_CONFIG}|${RISK_ENV_BACKEND:-${DEFAULT_ENV_BACKEND}}|${RISK_CUDA_VISIBLE_DEVICES:-${DEFAULT_CUDA_VISIBLE_DEVICES}}|${RISK_EXTRA_ARGS:-}"
  "1|formation_mappo_pure|formation|${FORMATION_TOTAL_UPDATES}|${FORMATION_CONFIG}|${FORMATION_ENV_CONFIG}|${FORMATION_ENV_BACKEND:-${DEFAULT_ENV_BACKEND}}|${FORMATION_CUDA_VISIBLE_DEVICES:-${DEFAULT_CUDA_VISIBLE_DEVICES}}|${FORMATION_EXTRA_ARGS:-}"
)

write_summary_header

if ! check_required_files \
  "${GOAL_CONFIG}" "${GOAL_ENV_CONFIG}" \
  "${COVERAGE_CONFIG}" "${COVERAGE_ENV_CONFIG}" \
  "${RISK_CONFIG}" "${RISK_ENV_CONFIG}" \
  "${FORMATION_CONFIG}" "${FORMATION_ENV_CONFIG}"; then
  exit 2
fi

if [[ "${NOTIFY_ONLY}" == "1" ]]; then
  if notify_email "[Wayffusion] MAPPO pure all4 notification test" "Wayffusion MAPPO pure all4 notification test.
...
run_one_job() {
  local row="$1"
  IFS='|' read -r enabled label task total_updates config env_config env_backend cuda_visible_devices extra_args <<< "${row}"
  local run_log="${QUEUE_LOG_ROOT}/${label}.log"
  local status_tsv="${QUEUE_LOG_ROOT}/${label}.status.tsv"
  local run_dir="outputs/training/mappo/${RUN_TIMESTAMP}/${label}"
...
log "parallel MAPPO pure all4 specialists started"
log "repo_root=${REPO_ROOT}"
log "run_timestamp=${RUN_TIMESTAMP}"
log "queue_log_root=${QUEUE_LOG_ROOT}"
log "email_to=${EMAIL_TO}"
log "eval_episodes=${EVAL_EPISODES}"
log "note=all jobs start from random initialization; no BC checkpoint or PPO checkpoint is loaded"

declare -A pids
for row in "${JOBS[@]}"; do
  IFS='|' read -r _enabled label task total_updates config env_config env_backend cuda_visible_devices extra_args <<< "${row}"
  log "start ${label}: task=${task}; updates=${total_updates}; config=${config}; env=${env_config}; cuda=${cuda_visible_devices}"
  run_one_job "${row}" &
  pids["${label}"]="$!"
...
for row in "${JOBS[@]}"; do
  ...
  if [[ -f "${status_tsv}" ]]; then
    ...
    end_time="$(date --iso-8601=seconds)"
    duration_sec="0"
    run_dir="outputs/training/mappo/${RUN_TIMESTAMP}/${label}"
  fi
  case "${status}" in
  ...
  append_summary "${label}" "${status}" "${exit_code}" "${start_time}" "${end_time}" "${duration_sec}" "${task}" "${total_updates}" "${EVAL_EPISODES}" "${config}" "${env_config}" "${run_dir}" "${run_log}"
...
subject="[Wayffusion] MAPPO pure all4 ${queue_status}: ${completed_runs} completed, ${failed_runs} failed"
body="Wayffusion MAPPO pure all4 specialists finished.
